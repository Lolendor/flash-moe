...
// ============================================================================
// Transpose column-major [M_padded, N] → row-major [M, N]
// NAX GEMM outputs column-major: element (m,n) at offset m + n*M_padded
// Batched prefill needs row-major: element (m,n) at offset m*N + n
// Each thread handles one element.
// ============================================================================

kernel void nax_transpose_cm_to_rm(
    const device float* src        [[buffer(0)]],   // column-major [M_padded, N]
    device float*       dst        [[buffer(1)]],   // row-major [M, N]
    constant uint& M_actual        [[buffer(2)]],   // actual M (batch_n)
    constant uint& N_dim           [[buffer(3)]],   // N (out_dim)
    constant uint& M_padded        [[buffer(4)]],   // padded M (stride in src)
    uint2 tid [[thread_position_in_grid]])
{
    uint m = tid.y;
    uint n = tid.x;
    if (m < M_actual && n < N_dim) {
        dst[m * N_dim + n] = src[m + n * M_padded];
    }
}
