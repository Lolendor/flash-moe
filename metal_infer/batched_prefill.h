...
static int prefill_k0_token_first(
    ...
    int token_idx;
    for (token_idx = 0; token_idx < num_prefill; token_idx++) {
        if (atomic_load(&g_prefill_abort)) {
            fprintf(stderr, "[prefill] aborted at token %d/%d (token-first)\n", token_idx, num_prefill);
            break;
        }
        @autoreleasepool {
...
static void debug_compare_full_attn_layer(
    ...
    // Step 8: o_proj
    fprintf(stderr, "  [cmp] L%d o_proj weights: w=%p s=%p b=%p in=%d out=%d\n",
            layer, (void*)lc->o_w, (void*)lc->o_s, (void*)lc->o_b,
            q_head_dim, HIDDEN_DIM);
    float *o_proj_out = (float *)calloc(HIDDEN_DIM, sizeof(float));
    fast_dequant_matvec(lc->o_w, lc->o_s, lc->o_b,
...
static int prefill_k0_layer_first(
    ...
    for (int layer = 0; layer < NUM_LAYERS; layer++) {
        if (atomic_load(&g_prefill_abort)) {
            fprintf(stderr, "[prefill] aborted at layer %d/%d (layer-first)\n", layer, NUM_LAYERS);
            aborted_layer = 1;
            break;
        }
        @autoreleasepool {
            ...
            if (is_full &&
                ...
                // In our codebase weights always come from the weight file
                int full_wf_only = 1;
                int q_proj_dim = NUM_ATTN_HEADS * HEAD_DIM * 2;
                int q_head_dim = NUM_ATTN_HEADS * HEAD_DIM;
                int kv_dim = NUM_KV_HEADS * HEAD_DIM;
            ...
            if (!used_batched_layer &&
                ...
                // In our codebase weights always come from the weight file
                int linear_wf_only = 1;

                if (linear_layer_idx >= 0 && linear_layer_idx < cfg.num_linear_layers &&
                    linear_wf_only &&
                    lc->input_norm_w && lc->post_attn_norm_w &&
    ...
    // Layer-first processes all tokens at each layer, so partial layer completion
    // leaves tokens in an inconsistent state — return 0 to signal abort.
    return aborted_layer ? 0 : num_prefill;
}
